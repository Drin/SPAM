...
def main(iso_ids, iso_labels, iso_data_cpu,
   ...
   while (curr_up < num_up and end_ndx < num_isolates and start_ndx < end_ndx):
      ...
      OHClusterer = Clusterer.Clusterer(threshold, use_transform=use_trans,
                                        ontology=clust_ontology)
      cluster_t = time.time()
      ...
      if (clust_ontology is not None):
         ...
         backup_clusters.extend(tmp_clusters)
         persist_clusters = copy.deepcopy(backup_clusters)
...
if (__name__ == '__main__'):
   ...
   for test_conf in configurations:
      for test_num in range(3):
         ...
         main(iso_ids, iso_labels, iso_data_cpu, iso_sim_mapping,
              sim_matrix_ndx, use_trans=False, clust_ontology=tmp_ontology,
   ...
   for test_conf_ndx in range(len(configurations)):
      ...
      for test_num in range(3):
         main(iso_ids, iso_labels, iso_data_cpu, iso_sim_mapping,
              sim_matrix_ndx, use_trans=False, init_size=test_conf[0],
